...
// ────────────────────────────────────────────────
//  MONITOR ATTIVI (stato attuale dei segnali precedenti)
// ────────────────────────────────────────────────
    ...
    for (const t of tickers) {
      ...
      for (const levelKey of ['ULTRA', 'SUPER', 'BIG']) {
        ...
        if (signal) {
          ...
          break; // solo il livello più alto
        }
      }
    }
  } catch (err) {
...
// ────────────────────────────────────────────────
console.log(
  `🚀 REVERSAL EXPLOSION SCANNER v10.1 (CON PERPS + FIX BOOK/LONG) avviato - ogni ${CONFIG.SCAN_INTERVAL_MIN} min`
);
